...
double ourGame::computeHeuristicValue(int player){
  ...
  int startPiece;//0 for empty , 1 for blocked, 2 for opponent, 3 for our ring
  //
  for(int i=0;i<rows+1;i++){
    valuesForConsecutiveMarkers.push_back(0);
    valuesForConsecutiveMarkers2.push_back(0);
  ...
  for(int i=0;i<rows;i++){
    for(int j=0;j<rows;j++){
      if(!board[i][j].canBeUsed){
        //if not on board
        if(count!=0){
          if(count==this->markersNeededToRemove){
              score+=30;
            }
        ...
        if(board[i][j].player!=player){
          if(count!=0){
            ...
            if(count==this->markersNeededToRemove&&board[i][j].player==otherPlayer&&board[i][j].containsRings){
              score-=80;
            }
            valuesForConsecutiveMarkers[count]++;
            if(doesMyLineContainRing){
  ...
  int weight[] = {0,1,3,9,27,81,100,100,100,100,100,100,100,100};
  ...
  if(player==1){
    score+=10000*(myRingsInitial-this->playerOneRingsOnBoard);//100000
    score-=20000*(opponentRingsInitial-this->playerTwoRingsOnBoard);//200000
    // score+=2*this->playerOneMarkersOnBoard;
  }else{
    score+=10000*(myRingsInitial-this->playerTwoRingsOnBoard);
    score-=20000*(opponentRingsInitial-this->playerOneRingsOnBoard);////teen zeros aur the
    ...
    // score+=tempV.size();
    // tempV = tempPlayer.selectAndMove(3-player,this);
    // score-=tempV.size();
