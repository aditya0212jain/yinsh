...
void ourPlayer::play(){
  ...
  clock_t t1,t2;
  t1 = clock();
  if(this->game->rows/2==6&&this->markersNeededToRemove==6){
      set_max_depth=3;
    }
  if(this->playerNumber==1){
    ...
    transitionMove m = idMinimax(set_max_depth,seconds);//max_depth,time
    cout<<m.move<<endl;
  ...
  while(!this->game->ended()){
    ...
    transitionMove m = idMinimax(set_max_depth,seconds);//minimax(0,true,-INFINITY,INFINITY,depth)
    cout<<m.move<<endl;
    moveDecider(this->playerNumber,m.move,this->game);
    // this->game->printBoard();
